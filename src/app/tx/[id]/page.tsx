"use client";

import Link from "next/link";
import { useParams } from "next/navigation";
import { TransactionDetails } from "@/components/tx/TransactionDetails";
import { usePaymentStore } from "@/store/paymentStore";

export default function TxDetailsPage() {
  const params = useParams<{ id: string }>();
  const id = params.id;
  const tx = usePaymentStore((s) => s.transactionsById[id]);

  return (
    <div className="flex flex-1 flex-col bg-zinc-50 text-zinc-950 dark:bg-black dark:text-zinc-50">
      <header className="border-b border-zinc-200 bg-white/70 backdrop-blur dark:border-zinc-800 dark:bg-black/40">
        <div className="mx-auto flex w-full max-w-5xl items-center justify-between px-4 py-4">
          <Link href="/" className="text-sm font-semibold tracking-tight">
            Payment Gateway Demo
          </Link>
          <nav className="flex items-center gap-4 text-sm">
            <Link className="underline underline-offset-4" href="/pay">
              Pay
            </Link>
            <Link className="underline underline-offset-4" href="/history">
              History
            </Link>
          </nav>
        </div>
      </header>

      <main className="mx-auto w-full max-w-5xl flex-1 px-4 py-10">
        {!tx ? (
          <section className="rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-black">
            <h1 className="text-xl font-semibold tracking-tight">Transaction not found</h1>
            <p className="mt-2 text-sm text-zinc-600 dark:text-zinc-400">
              This transaction doesn’t exist in your local history yet.
            </p>
            <div className="mt-4 flex flex-wrap gap-3 text-sm">
              <Link className="underline underline-offset-4" href="/history">
                Go to history
              </Link>
              <Link className="underline underline-offset-4" href="/pay">
                Make a payment
              </Link>
            </div>
          </section>
        ) : (
          <TransactionDetails tx={tx} />
        )}
      </main>
    </div>
  );
}
